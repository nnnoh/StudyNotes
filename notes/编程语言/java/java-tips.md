...
一个类扩展了一个超类，同时实现了一个接口，并从超类和接口继承了相同的方法。在这种情况下， 只会考虑超类方法， 接口的所有默认方法都会被忽略。这是“ 类优先” 规则。

当类继承不同接口提供的同名同参不同默认实现的方法。Java 编译器会报告一个错误，让程序员来解决这个二义性。只需要在子类中提供该方法的实现即可。

只要至少有一个接口提供了一个实现， 编译器就会
报告错误， 而程序员就必须解决这个二义性。

> 同名同参方法的返回值需要一致、或能够相互转换（父子类关系）。否则编译错误

当父类和接口中有同名同参的方法实现时，可通过 `名称.super` 引用相应的方法。
...
![Alibaba Coding Guidelines](https://gitee.com/nnnoh/image-hosting-service/raw/master/img/2020/image-20200528103334850.png)
...
[Java中 a+=b和a=a+b有什么区别？_m0_38022029的博客-CSDN博客](https://blog.csdn.net/m0_38022029/article/details/79606104)
